...
// ---------------------------------------------------------------------------
// Config and font atlas (Unicode codepoints)
// ---------------------------------------------------------------------------

// When no font_path is set in the config file, Windows builds use this
// Scoop-installed Maple Mono path so Chinese and Nerd Font glyphs work
// out of the box.  Override with font_path in %APPDATA%\ghostling\config.
#ifdef _WIN32
#define GHOSTLING_DEFAULT_FONT_PATH \
    "C:\\Scoop\\apps\\Maple-Mono-NF-CN\\7.9\\MapleMono-NF-CN-Regular.ttf"
#endif
...
// Write the default per-user config file path into out (UTF-8 path).
// Windows: %APPDATA%\ghostling\config
// Unix:    $HOME/.config/ghostling/config
...
// Parse key = value lines.  Empty lines and # comments are ignored.
// Recognized keys: font_path, font_size (logical points, default 16).
...
static void config_load(AppConfig *cfg)
{
    cfg->font_path[0] = '\0';
    cfg->font_size = 16;
...
#ifdef _WIN32
    // If the user has no config (or no font_path in it), try the default
    // Maple Mono install so CJK and NF glyphs work without setup.
    if (!cfg->font_path[0]) {
...
// raylib packs only the codepoints you pass into the font atlas.  NULL, 0
// loads ~95 ASCII glyphs, which is why everything else showed as '?'.
// These ranges cover Latin, box-drawing, CJK Extension A + Unified, and
// common fullwidth / CJK punctuation for terminal work.
...
static int *build_terminal_codepoints(int *out_count)
{
    const int cap = 32768;
...
#define R(lo, hi) \
    ...
    R(0x20, 0x7E);       // ASCII printable
    R(0xA0, 0x024F);     // Latin-1 supplement through Latin Extended-B
    R(0x0300, 0x036F);   // Combining diacritical marks
    R(0x2000, 0x206F);   // General punctuation
    R(0x20A0, 0x20CF);   // Currency symbols
    R(0x2100, 0x214F);   // Letterlike / Number forms
    ...
    R(0x3000, 0x30FF);   // CJK symbols, punctuation, Hiragana, Katakana
    R(0x31F0, 0x31FF);   // Katakana phonetic extensions
    R(0x3200, 0x32FF);   // Enclosed CJK
    R(0x3300, 0x33FF);   // CJK compatibility
    R(0x3400, 0x4DBF);   // CJK Unified Extension A
    R(0x4E00, 0x9FFF);   // CJK Unified Ideographs
    R(0xFE10, 0xFE1F);   // Vertical punctuation
    R(0xFE30, 0xFE4F);   // CJK compatibility forms
    R(0xFF00, 0xFFEF);   // Halfwidth and fullwidth forms
#undef R
...
// Load a monospace font with an explicit glyph set.  Tries disk path first
// when path is non-empty and readable; otherwise the embedded JetBrains
// buffer (which lacks CJK — disk font is required for Chinese).
static Font load_terminal_font(const char *path,
                               const unsigned char *embed, int embed_size,
                               int font_size_px, int *codepoints, int cp_count)
{
    if (path && path[0] && file_is_readable(path)) {
        ...
        if (f.glyphCount > 0 && f.texture.id > 0)
            return f;
        fprintf(stderr, "ghostling: LoadFontEx failed for \"%s\", using embedded font\n",
                path);
    } else if (path && path[0]) {
...
int main(int argc, char *argv[])
{
    ...
    // Build Unicode codepoint list for the atlas, then load the font at
    // native pixel size so glyphs map cleanly to the framebuffer.
    int cp_count = 0;
    ...
    Font mono_font = load_terminal_font(font_path_try, font_jetbrains_mono,
